            ...
            >
              <div className="p-6 border-b border-slate-100 dark:border-slate-700 flex items-center justify-between">
                <h3 className="text-xl font-bold flex items-center gap-2">
                   換人: <PlayerCard player={player} size="sm" className="pointer-events-none" />
                </h3>
                      ...
                      <div className="bg-slate-50 dark:bg-slate-800/50 p-6 rounded-3xl border border-slate-100 dark:border-slate-700" data-html2canvas-ignore="true">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {waitingPlayers.map(p => (
                            <div key={p.id} className="bg-white dark:bg-slate-700 px-3 py-1.5 rounded-xl shadow-sm text-sm font-bold text-slate-700 dark:text-slate-200 border border-slate-100 dark:border-slate-600">
                              {p.name}
                            </div>
                          ))}
                        </div>
            ...
            {waitingPlayers.length > 0 && (
              <div className="w-full max-w-screen-2xl mt-12 bg-slate-900 p-8 rounded-3xl border border-slate-800">
                ...
                <div className="flex flex-wrap gap-4">
                  {waitingPlayers.map(p => (
                    <div key={p.id} className="bg-slate-800 px-5 py-3 rounded-2xl shadow-inner text-lg font-bold text-white border border-slate-700">
                      {p.name}
                    </div>
                  ))}
                </div>
