...
const PlayerCard: React.FC<PlayerCardProps> = ({ 
  ...
  const colorClass = isMale ? COLORS.male : COLORS.female;
  ...
  return (
    ...
    `}>
      <div className={`
        ...
        ${isMale ? 'bg-blue-200 dark:bg-blue-800' : 'bg-pink-200 dark:bg-pink-800'}
      `}>
        <User size={size === 'sm' ? 12 : 16} className={isMale ? 'text-blue-600 dark:text-blue-300' : 'text-pink-600 dark:text-pink-300'} />
      </div>
      <span className={`font-bold tracking-wide ${autoWidth ? '' : 'truncate max-w-[100px]'}`}>
        {player.name}
      </span>
